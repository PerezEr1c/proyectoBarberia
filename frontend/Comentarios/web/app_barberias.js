...
async function buscarBarberia() {
  ...
  try {
    const res = await fetch(
      API_BASE_BARBERIA + ENDPOINTS_BARBERIA.read_one.replace("{id}", id)
    );
    if (res.ok) {
...
async function actualizarBarberia() {
  ...
  const direccion = document
    .getElementById("direccionBarberiaAccion")
    .value.trim();
  const telefono = document
    .getElementById("telefonoBarberiaAccion")
    .value.trim();
  ...
  try {
    const res = await fetch(
      API_BASE_BARBERIA + ENDPOINTS_BARBERIA.update.replace("{id}", id),
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(barberia),
...
async function eliminarBarberia() {
  ...
  try {
    const res = await fetch(
      API_BASE_BARBERIA + ENDPOINTS_BARBERIA.delete.replace("{id}", id),
      {
        method: "DELETE",
      }
    );
